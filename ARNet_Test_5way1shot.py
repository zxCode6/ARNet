...
model_trained = './results/A_PRNet_miniImageNet_ResNet256F_5Way_1Shot_K3/model_best.pth.tar'
...
parser.add_argument('--query_num', type=int, default=20, help='the number of queries')
parser.add_argument('--neighbor_k', type=int, default=3, help='the number of k-nearest neighbors')
parser.add_argument('--lr', type=float, default=0.0005, help='learning rate, default=0.005')
...
parser.add_argument('--t',type=int,default=0,help='t')
...
def validate(val_loader, model, criterion, epoch_index, F_txt):
	...
	for episode_index, (query_images, query_targets, support_images, support_targets) in enumerate(val_loader):
		...
		target = torch.cat(query_targets, 0).long()
		target = target.cuda()
...
repeat_num = 3       # repeat running the testing code several times
