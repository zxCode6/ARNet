...
parser.add_argument('--time',default=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
...
parser.add_argument('--outf', default='./results/A_PRNet')
...
parser.add_argument('--episode_val_num', type=int, default=600, help='the total number of evaluation episodes')
...
parser.add_argument('--lr', type=float, default=0.00005, help='learning rate, default=0.005')
...
parser.add_argument('--t',type=int,default=10,help='t')
parser.add_argument('--h',type=int,default=1,help='h')
parser.add_argument('--p',type=int,default=0.1,help='h')
opt = parser.parse_args()
opt.cuda = True
cudnn.benchmark = True
shotNum=opt.shot_num
t=opt.t
h=opt.h
p=opt.p
...
def train(train_loader, model, criterion, optimizer, epoch_index, F_txt):
	...
	for episode_index, (query_images, query_targets, support_images, support_targets) in enumerate(train_loader):
		...
		# Convert query and support im ages
		...
		output_global,output_local = model(input_var1, input_var2)
		...
		prec1, _ = accuracy(output_global+output_local, target, topk=(1,3))
		# prec2, _ =accuracy(output_local,target,topk=(1,3))
		prec=prec1
		losses.update(loss.item(), query_images.size(0))
		top1.update(prec[0], query_images.size(0))
...
model = DN4Net.define_DN4Net(which_model=opt.basemodel, num_classes=opt.way_num, neighbor_k=opt.neighbor_k, norm='batch',
	init_type='normal', use_gpu=opt.cuda,shot=shotNum,parameter_t=t,parameter_h=h)
